namespace Water2D
{
	...
	public class Water2DEditor : Editor {
        ...
        SerializedProperty GlossOffset;
        ...
        internal void OnEnable(){
            ...
            GlossOffset = serializedObject.FindProperty("GlossOffset");
        ...
        void checkTargetAndSamlpeSize(int sampleID = -1)
        {
            if(sampleID > 2 && 
                (EditorUserBuildSettings.activeBuildTarget == BuildTarget.iOS) ||
                (EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)) // then is 256 or higher
            EditorGUILayout.HelpBox("Sample Texture size is bigger than recommended for mobile devices! For better performance please downsize in 'Project Settings > Water2D'", MessageType.Warning);
        ...
        public override void OnInspectorGUI () {
            ...
            if (WaterStyle.enumValueIndex != (int)tipes) {
                ...
                //report style change
                SpawnersManager.ReloadAllSpawners();
            }
            ...
            string vv = AssetUtility.LoadProperty("w2d_version", m_CustomSettings);
            EditorGUILayout.LabelField("Version:" + ((vv != null) ? vv : "n/a") + " | " + "Render pipeline:" + WSpawner.Water2DRenderType, textSubHeaderStyle3);
            EditorGUILayout.EndHorizontal();
            ...
            */
            ...
            int _sampleSize = AssetUtility.LoadPropertyAsInt("SampleID", m_CustomSettings);
            checkTargetAndSamlpeSize(_sampleSize);
            ...
            if (_material != null)
            {
                ...
                */
                ...
                else if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Refracting) // REFRACTING SHADER
                {
                    ...
                    EditorGUILayout.Slider(Intensity, 0f, 3f, new GUIContent("Intensity"));
                    EditorGUILayout.PropertyField(StrokeColor, new GUIContent("Fresnel Color"));
                    EditorGUILayout.Slider(AlphaCutOff, 0f, 1f, new GUIContent("Alpha CutOff"));
                ...
                else if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Glossy) // Glossy SHADER
                {
                    if (Selection.transforms.Length < 2)
                    {
                        Blending.boolValue = EditorGUILayout.Toggle("Blending Colors", Blending.boolValue);
                    ...
                    EditorGUILayout.PropertyField(FillColor, new GUIContent("Fill Color"));
                    EditorGUILayout.PropertyField(StrokeColor, new GUIContent("Fresnel Color"));
                    EditorGUILayout.Slider(AlphaCutOff, 0f, 0.99f, new GUIContent("Alpha CutOff"));
                    EditorGUILayout.Slider(AlphaStroke, 0.5f, 1.6f, new GUIContent("Gloss Magnitude"));
                    EditorGUILayout.Slider(GlossOffset, 0.2f, 3f, new GUIContent("Gloss Offset"));
                }
                else if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Refract_Glossy) // REFRACTING GLOSSY SHADER
                {
                    Blending.boolValue = EditorGUILayout.Toggle("Blending Colors", Blending.boolValue);
                    EditorGUILayout.PropertyField(FillColor, new GUIContent("Tint Color"));
                    EditorGUILayout.Slider(Intensity, 0f, 3f, new GUIContent("Intensity"));
            ...
            if (foldoutEmissionFiller && WSpawner.Water2DEmissionType == Water2D_Spawner.EmissionType.FillerCollider)
            {
                ...
                EditorGUILayout.Slider(SizeDropObject, 1f, 8f, new GUIContent("Start Size"));
            ...
            if (foldoutPhysics)
            {
                ...
                EditorGUILayout.Slider(ColliderSize, .0001f, .1f, new GUIContent("Collider Size"));
                EditorGUILayout.PropertyField(AutoMass, new GUIContent("Use auto Mass"));
                
                if (AutoMass.boolValue) GUI.enabled = false;
        ...
        void OnSceneGUI()
        {
            ...
            if (WSpawner)
            {
               ...
               // EditorUtility.SetSelectedRenderState(WSpawner.GetComponent<MeshRenderer>(), EditorSelectedRenderState.Hidden);
                ...
                var fmh_1363_25_638649671839890107 = Quaternion.identity; newHandlePoint = (lTransform.position - Handles.FreeMoveHandle(
                ...
                var fmh_1402_91_638649671839909804 = Quaternion.identity; Vector3 newFreeHandlePoint = (Handles.FreeMoveHandle(lTransform.position,
                ...
                if (GUILayout.Button("Rebuild"))
                {
                    ...
                    if (Application.isPlaying)
                    {
                        wasRunning = WSpawner.IsSpawning;
                        WSpawner.StopSpawning();
                        WSpawner.SetupParticles(wasRunning);
