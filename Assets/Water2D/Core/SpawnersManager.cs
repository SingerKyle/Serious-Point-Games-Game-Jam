namespace Water2D
{
    ...
    public class SpawnersManager : MonoBehaviour
    {
        ...
        void reloadAllSpawners()
        {
            ...
            // Check if there are any Water2D_Spawner objects found
            if (s.Length == 0)
            {
                Debug.LogWarning("No Water2D_Spawner objects found!");
                return;
            }

            // Initialize caches with the first spawner's values (if available)
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                // Count each style and report multiple spawners use to the mesh and shader
                ...
                // Check if any properties are distinct from the first spawner
            ...
            // Report multiple spawners use to the mesh and the shader
            ...
            else
            {
                if (isDistinct)
                    useMultipleStylesPerScene = true;
                else // Force multiple when you have the same style but distinct Fresnel color
       ...
       static Color GammaToLinear(Color c)
        ...
        public static void ChangeSpawnerValues(Water2D_Spawner _spawner)
        {
            ...
            // Look for spawner already in the scene
            ...
            // Ensure the HashID is valid before using it
            if (_spawner.HashID >= 0 && _spawner.HashID < instance.spawnersID.Count)
            {
                ...
                instance.LensSizeByID[_spawner.HashID] = _spawner.LensMag;
            ...
            {
                Debug.LogError("HashID is out of bounds! No valid spawner array index.");
            }
        }
